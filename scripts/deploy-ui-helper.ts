...
const main = async function () {
  const VestUiHelper = await hre.ethers.getContractFactory("VestUiHelper");
  ...
  // implementations
  const vestUiHelper = await VestUiHelper.deploy(args[0], args[1], args[2]);

  console.log("VestUiHelper", vestUiHelper.target);

  console.log(
    await vestUiHelper.getLPLockDetails(
