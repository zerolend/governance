...
import { BalanceLeaf, BalanceTree } from "./airdrop-utils/merkle-tree/BalanceTree";
...
describe("Airdrop Tests", async () => {
  ...
  beforeEach("Deploy Airdrop Contracts", async () => {
    ...
    leaves = [user1.address, user2.address].map(
      (account, i) => ({
    ...
    await airdropRewarder.setMerkleRoot(tree.getHexRoot(), airdropToken.target);
  });
  ...
  it("Cannot claim with wrong amount", async () => {
    ...
    await expect(
      airdropRewarder
        .connect(user1)
        .claim(leaves[0].account, airdropToken.target, parseEther("0.5"), proof)
  ...
  it("Cannot claim with other user", async () => {
    ...
    await expect(
      airdropRewarder
        .connect(user1)
        .claim(leaves[1].account, airdropToken.target, parseEther("2"), proof)
    ).to.be.revertedWithCustomError(airdropRewarder, "InvalidMerkleProof");
  });
  ...
  it("Can claim successfully", async () => {
    ...
    await expect(
      ...
      .withArgs(leaves[0].account, airdropToken.target, parseEther("1"));
  });
  ...
  it("can claim for multiple users", async () => {
    ...
    await expect(
      ...
      .to.emit(airdropRewarder, "RewardsClaimed")
      .withArgs(leaves[1].account, airdropToken.target, parseEther("2"));
  });
});
